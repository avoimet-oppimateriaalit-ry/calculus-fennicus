...
$q=0$. Siis $p_1=p_2$, eli ehdot \eqref{interpolaatioehdot} t‰ytt‰v‰ polynomi on 
yksik‰sitteinen. \loppu

Lagrangen interpolaatiopolynomin approksimaatiovirheelle p‰tee seuraava tulos, joka muistuttaa 
Taylorin polynomien virhekaavaa (Lause \ref{Taylor}). J‰ljemp‰n‰ n‰hd‰‰nkin, ett‰ n‰m‰ kaksi
tulosta ovat erikoistapauksia yleisemm‰st‰ interpolaatiopolynomien virhekaavasta 
(Lause \ref{usean pisteen Taylor}). Jatkossa sanotaan ‰‰rellisen pisteistˆn $X$ 
\kor{viritt‰m‰ksi} v‰liksi suljettua v‰li‰ $[a,b]$, miss‰ $a = \min \{x \mid x \in X\}$ ja 
$b = \max \{x \mid x \in X\}$.
\begin{Lause} \label{Lagrangen interpolaatiovirhe}
Olkoon $f\,$ $n+1$ kertaa derivoituva avoimella v‰lill‰ $A\supset[a,b]$, miss‰ $[a,b]$ on
pisteen $x$ ja erillisten pisteiden $x_0,\ldots,x_n$ viritt‰m‰ v‰li. T‰llˆin jos $p$ on $f$:n
Lagrangen interpolaatiopolynomi pisteiss‰ $x_0,\ldots,x_n$, niin jollakin $\xi \in (a,b)$
p‰tee virhekaava
\[
...
\ref{taylorin lause}. Ensinn‰kin jos $x\in\{x_0,\ldots,x_n\}$, on v‰itt‰m‰ tosi jokaisella 
$\xi \in (a,b)$, joten voidaan olettaa, ett‰ $x\notin\{x_0,\ldots,x_n\}$. Merkit‰‰n
\[
...
derivaatan nollakohta, on $g'(t)$:ll‰ ainakin $n+1$ nollakohtaa avoimella v‰lill‰ $(a,b)$. 
T‰llˆin $g''$:lla on ainakin $n$ nollakohtaa t‰ll‰ v‰lill‰, ja lopulta $g^{(n+1)}$:lla ainakin
yksi nollakohta $\xi\in (a,b)$. Mutta t‰llˆin
\begin{align*}
...
m‰‰r‰‰mistarkoituksessa) tarkoittaa $f$:n \kor{paloittain lineaarista} interpolaatiota.
\begin{figure}[H]
\begin{center}
\import{kuvat/}{kuvaipol-1.pstex_t}
\end{center}
\end{figure}
Lineaarisen interpolaation virhe interpolointiv‰lill‰ $[x_0,x_1]$ on Lauseen 
\ref{Lagrangen interpolaatiovirhe} mukaan enint‰‰n
\begin{align*}
...
\]
T‰m‰ arvio ei ole parannettavissa, sill‰ jos $f$ on toisen asteen polynomi (jolloin $f''(x)=$ 
vakio) ja interpolointipisteet ovat tasav‰liset, niin Lauseen
\ref{Lagrangen interpolaatiovirhe} virhekaavan mukaan virhearvion yl‰raja totetuu
...
\]
\begin{Exa}
V‰lill‰ $(0,\infty)$ m‰‰ritelty funktio $F$ toteuttaa ehdot
\[
...
\]
Halutaan m‰‰ritt‰‰ $F$:n arvot likim‰‰rin v‰lill‰ $[1,2]$ laskemalla ensin $F$ pisteiss‰ 
...
$f(0)\approx p(0)$. T‰ss‰ on $\{x_0,x_1,x_2\}=\{0.1,0.2,0.3\}$, joten
\begin{align*}
...
k‰tev‰ silloin, kun halutaan arvioida \pain{mittausvirheiden} interpolointitulokseen. 
Nimitt‰in jos virheellisten mittaustulosten $f(x_i)=\tilde{f}_i$ (oikea tulos $=f_i$)
...
\]
T‰ss‰ m‰‰ritelty virheen vahvistuskerroin $K(x)$ siis kertoo, kuinka paljon (itseis\-arvoltaan 
enint‰‰n tietyn suuruiset) mittausvirheet voivat pahimmillaan vahvistua pisteess‰ $x$.
\jatko \begin{Exa} (jatko) Jos oletetaan, ett‰ annettujen mittaustulosten virheet ovat enint‰‰n
$10^{-4}$ itseisarvoltaan, niin virheiden vaikutus pisteess‰ $x=0$ on enint‰‰n
\[ 
\abs{p(0) - \tilde{p}(0)} \le 10^{-4} \cdot (3+3+1) = 7 \cdot 10^{-4}. 
\]
Virheen vaikutus pisteess‰ $x=0$ voi siis pahimmassa tapauksessa olla $7$-kertainen
...
Osoittautuu, ett‰ taulukon sarakkeet m‰‰r‰ytyv‰t palautuvasti edellisen sarakkeen avulla. 
Nimitt‰in
\begin{equation} \label{Nevillen kaava}
\boxed{\ p^{(i,j)}(x)=\frac{(x_j-x)p^{(i,j-1)}(x)+(x-x_i)p^{(i+1,j)}(x)}{x_j-x_i} \quad}
\end{equation}
T‰m‰ palautuskaava (jonka voi perustella induktiolla --- 
...
sanotaan \kor{Nevillen kaavioksi}. Ennen tietokoneiden aikaa t‰llaisilla algoritmisilla 
keksinnˆill‰ oli huomattava k‰yt‰nnˆn merkitys, koska ne helpottivat k‰sinlaskua.} 
Seuraavassa esimerkki kaavaan \eqref{Nevillen kaava} perustuvasta 'laskemisen taiteesta'.
\begin{Exa} \label{Neville}
M‰‰ritell‰‰n $\phi(n)=n!/(\sqrt{2\pi n}\,e^{-n}n^n)$. Ekstrapoloi $\phi(100)$ arvoista 
$\phi(n)$, $n=5\ldots 10$, kun tiedet‰‰n, ett‰ $\phi(n)=f(1/n)$, miss‰ $f$ on sile‰ funktio 
v‰lill‰ $[0,1]$.
\end{Exa}
...
Esimerkiss‰ olennaista oli tieto, ett‰ voidaan kirjoitaa $\phi(n)=f(1/n)$, miss‰ $f$ on origon 
l‰hell‰ s‰‰nnˆllinen reaalifunktio. Ekstrapoloitaessa on siis osattava valita oikea muuttuja.
...
$x_i$. Interpolaatiopisteiden lukum‰‰r‰ $m$ voi siis yleistetyss‰ polynomi-interpolaatiossa
astetta $n$ olla mik‰ tahansa v‰lill‰ $1 \le m \le n+1$. Tapauksessa $m=n+1$ on ehtojen
\eqref{yleiset interpolaatioehdot} mukaan oltava $\nu_i=1\ \forall i$, jolloin kyseess‰ on 
tavallinen Lagrangen interpolaatio. Toisessa ‰‰rip‰‰ss‰ ($m=1$) on taas oltava $\nu_1=n+1$, 
...
Harj.teht.\,\ref{H-VIII-7: yleinen interpolaatio-ongelma}. Yksik‰sitteisyyden toteamiseksi
...
Olkoon $f\,$ $n+1$ kertaa derivoituva avoimella v‰lill‰ $A\supset[a,b]$, miss‰ $[a,b]$ on
pisteen $x$ ja erillisten pisteiden $x_0,\ldots,x_m$ viritt‰m‰ v‰li. T‰llˆin jos $p$ on
...
g(t)=f(t)-p(t)- Hw(t),\quad H=[f(x)-p(x)]/w(x).
\]
Koska $w^{(n+1)}(t)=(n+1)!\ \forall t$, niin virhekaava v‰itt‰‰, ett‰ $g^{(n+1)}(\xi)=0$ 
jollakin $\xi \in (a,b)$. Aiemmin t‰m‰ on osoitettu tapauksissa $m=1,\ \nu_1=n+1$ 
(Taylorin lause) ja $m=n+1,\ \nu_i=1$ (Lause \ref{Lagrangen interpolaatiovirhe}). 
Yleisemm‰ss‰kin tapauksessa on p‰‰ttely vastaava. Esimerkiksi olkoon 
\[ 
...
seuraa, ett‰ ainakin yhdess‰ pisteess‰ $\xi \in (a,b)$ on $g^{(2m)}(\xi)=g^{(n+1)}(\xi)=0$,
eli v‰ite on tosi oletetussa tapauksessa. (Yleinen tapaus sivuutetaan). \loppu

Edell‰ tarkastellussa tapauksessa, jossa $\nu_i=2,\ i=1\ldots m$, sanotaan polynomia $p$ 
funktion $f$ \kor{Hermiten interpolaatiopolynomiksi} pisteiss‰ $x_i$.
\setcounter{Exa}{0}
...
\end{align*}
jos $\abs{F^{(4)}(\xi)}\leq M$. V‰lill‰ $[1,2]$ on $\abs{F^{(4)}(x)}\leq 16/e$, joten
vaadittuun tarkkuuteen riitt‰‰:
\[
...
$F$ saavuttaa absoluuttisen minimiarvonsa v‰lill‰ $[1,2]$. Tiedet‰‰n lis‰ksi, ett‰
$F(0.5)=\sqrt{\pi}$, $F(1)=1$, $F(1.5)=\sqrt{\pi}/2$ ja $F(2)=1$. Laske $F$:n minimikohdalle
ja -arvolle likiarvo k‰ytt‰en a) kvadraattista interpolaatiota pisteiss‰ $\{0.5,1,1.5\}$, \
b) kvadraaattista interpolaatiota pisteiss‰ $\{1,1.5,2\}$, \ c) kolmannen asteen 
interpolaatiota kaikissa nelj‰ss‰ pisteess‰.
...
\item
Heilurilla varustettu sein‰kello j‰t‰tt‰‰ vuorokaudessa $5$ min $24$ s. Kelloa rukataan 
kiert‰m‰ll‰ heilurin p‰‰ss‰ olevaa ruuvia kiinni $5$ t‰ytt‰ kierrosta (jolloin heilurin varsi
hieman lyhenee). Rukkauksen j‰lkeen havaitaan kellon edist‰v‰n $3$ min $36$ s vuorokaudessa.
\ a) Miten kelloa on seuraavaksi rukattava? \ b) Arvioi, kuinka tarkaksi kellon k‰ynti tuli
a-kohdan rukkauksella, jos kellon suhteellinen edist‰m‰ on likim‰‰rin
$f(x)=(1-\frac{x}{400})^{-1/2}-1$, miss‰ $x=$ ruuvin kiertym‰ (kierroksina) oikeasta 
s‰‰tˆasennosta mitattuna.
...
\item (*)
Olkoot $x_0, \ldots, x_n$ erillisi‰ pisteit‰ ja olkoot luvut $y_i\in\R,\ i= 0 \ldots n$ 
annettuja. Halutaan lˆyt‰‰ funktio muotoa
\[
F(x)=\sum_{j=0}^n c_j e^{jx}, \quad c_j\in\R
\]
siten, ett‰ p‰tee $F(x_i)=y_i,\ i=0 \ldots n$. N‰yt‰, ett‰ ongelmalla on yksik‰sitteinen
ratkaisu.

\item (*)
Lukujono $\seq{a_n}$, miss‰ $a_n = \sum_{k=1}^n k^{-5/4},\ n=1,2, \ldots\,$, suppenee, mutta
niin hitaasti, ett‰ raja-arvoa ei voi m‰‰r‰t‰ suoraan summeeraamalla. Voidaan kuitenkin
otaksua, ett‰ suurilla $n$:n arvoilla p‰tee
\[
a_n=a+n^{-1/4}(c_0+c_1n^{-1}+c_2n^{-2}+ \ldots)\,. 
\]
M‰‰rit‰ t‰m‰n oletuksen perusteella raja-arvo $a$ likim‰‰rin ekstrapoloimalla tuloksista
